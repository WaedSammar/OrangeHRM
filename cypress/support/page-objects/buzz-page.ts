...
class BuzzPage {
  ...
  static verifyPosterName(employeeInfo: IEmployeeInfo, postIndex: number = 0) {
    ...
    cy.get(this.LOCATORS.postEmpName).eq(postIndex).invoke(HTML_TAGS.text).should('eq', fullName)
  }
  ...
  static verifyToastAlert(message: string) {
    cy.get(this.LOCATORS.toastAlert).should('be.visible').and('contain.text', message)
  }
  ...
  static verifyDateAndTime(createdAt?: string, postIndex: number = 0) {
    ...
    const postOneMinuteAgo = dayjs(createdAt).subtract(1, 'minute').format(TIME_FORMATS.POST_TIME)
